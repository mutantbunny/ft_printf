...
/*   Updated: 2022/05/26 22:37:34 by gmachado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf_bonus.h"

int write_hex_left_justified(char *start, int len, t_format format, int num_zeros)
{
	int num_spaces;
	int copied;
...
int write_hex_right_justified(char *start, int len, t_format format, int num_zeros)
{
	int num_spaces;
	int copied;
...
int write_hex_padded(char *start, int len, t_format format)
{
	int remaining;
	int num_zeros;
	...
	if (format.flags & JUSTIFY_LEFT)
		return(write_hex_left_justified(start, len, format, num_zeros));
	else
		return(write_hex_right_justified(start, len, format, num_zeros));
}
